 ...
 */
inline fun <reified T> Set<T>.copyOf() = setOf(*this.toTypedArray())
 ...
 */
fun <T> Field.delegate(): ReadWriteProperty<Any, T> = FieldDelegate(this)
...
private class FieldDelegate<T>(private val field: Field) : ReadWriteProperty<Any, T> {
	...
	override fun getValue(thisRef: Any, property: KProperty<*>) = field.get(thisRef) as T
 ...
 */
fun toRadians(angle: Float) = angle / 180.0f * PI

class BlockPosNBTDelegate(private val tagName: String) : ReadWriteProperty<ItemStack, BlockPos?> {
	override fun getValue(thisRef: ItemStack, property: KProperty<*>) =
		thisRef.getChildTag(tagName)?.let(NBTUtil::readBlockPos)
...
fun modLoc(path: String) = ResourceLocation(HardcoreDungeons.ID, path)